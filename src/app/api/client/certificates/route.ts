...
export async function GET() {
  ...
  const certs = await prisma.serviceCertificate.findMany({
    where: { companyId: dbUser.companyId, status: { in: ["published","generated"] } },
    orderBy: { createdAt: "desc" },
    include: { vehicle: true, serviceOrder: { select: { folio: true } } },
  });
